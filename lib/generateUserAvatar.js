...
async function generateUseAvatar(userData) {
  try {
    ...
    await avatar
      ...
      .then(buffer => fs.writeFileSync(`tmp/avatar-${imageName}.png`, buffer));

    const tempAvatarPath = path.join(TEMP_DIR, `avatar-${imageName}.png`);
    const avatarPath = path.join(IMG_DIR, `avatar-${imageName}.png`);
